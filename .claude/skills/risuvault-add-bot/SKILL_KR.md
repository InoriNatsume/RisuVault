...
5. `add` 완료 후 `project_work/<이름>/`에 평문 파일이 이미 생성되어 있습니다 — 별도의 unlock 없이 바로 수정을 시작할 수 있습니다.
6. **커밋 전 반드시 `risuvault verify` 실행** (exit 0 필수). 위반 항목이 있으면 먼저 해결합니다.
7. 원격 백업 시 **중립 커밋 메시지** 사용 (프로젝트/캐릭터 이름 금지): `git add . && git commit -m "add 1 project" && git push`.
